...
**请求**:
```bash
curl -X POST "http://localhost:29090/api/ragent/ingestion/pipelines" \
...
**请求**:
```bash
curl -X POST "http://localhost:29090/api/ragent/ingestion/tasks/upload" \
  -F "pipelineId=1" \
...
**请求**:
```bash
curl "http://localhost:29090/api/ragent/ingestion/tasks/123"
```
...
API_BASE="http://localhost:29090/api/ragent"
