...
public class DuoJieChatClient implements ChatClient {
    ...
    public String provider() {
        return ModelProvider.DUO_JIE.getId();
    }

    @Override
    @RagTraceNode(name = "duojie-chat", type = "LLM_PROVIDER")
    public String chat(ChatRequest request, ModelTarget target) {
        ...
        try (Response response = httpClient.newCall(requestHttp).execute()) {
            if (!response.isSuccessful()) {
                ...
                log.warn("DuoJie 同步请求失败: status={}, body={}", response.code(), body);
                throw new ModelClientException(
                        "DuoJie 同步请求失败: HTTP " + response.code(),
            ...
            throw new ModelClientException("DuoJie 同步请求失败: " + e.getMessage(),
    ...
    @Override
    @RagTraceNode(name = "duojie-stream-chat", type = "LLM_PROVIDER")
    ...
    private void doStream(Call call, StreamCallback callback, AtomicBoolean cancelled) {
        try (Response response = call.execute()) {
            if (!response.isSuccessful()) {
                String body = readBody(response.body());
                throw new ModelClientException(
                        "DuoJie 流式请求失败: HTTP " + response.code() + " - " + body,
                        classifyStatus(response.code()),
                        response.code()
                );
            }
            ...
            if (body == null) {
                throw new ModelClientException("DuoJie 流式响应为空", ModelClientErrorType.INVALID_RESPONSE, null);
            }
            ...
            while (!cancelled.get()) {
                ...
                try {
                    ...
                    log.warn("DuoJie 流式响应解析失败: line={}", line, parseEx);
                }
            }
            if (!cancelled.get() && !completed) {
                throw new ModelClientException("DuoJie 流式响应异常结束", ModelClientErrorType.INVALID_RESPONSE, null);
            }
        } catch (Exception e) {
            callback.onError(e);
        }
    ...
    private AIModelProperties.ProviderConfig requireProvider(ModelTarget target) {
        if (target == null || target.provider() == null) {
            throw new IllegalStateException("DuoJie 提供商配置缺失");
        }
        if (target.provider().getApiKey() == null || target.provider().getApiKey().isBlank()) {
            throw new IllegalStateException("DuoJie API密钥缺失");
        }
    ...
    private String requireModel(ModelTarget target) {
        if (target == null || target.candidate() == null || target.candidate().getModel() == null) {
            throw new IllegalStateException("DuoJie 模型名称缺失");
        }
    ...
    private JsonObject parseJsonBody(ResponseBody body) throws IOException {
        if (body == null) {
            throw new ModelClientException("DuoJie 响应为空", ModelClientErrorType.INVALID_RESPONSE, null);
        }
    ...
    private String extractChatContent(JsonObject respJson) {
        if (respJson == null || !respJson.has("choices")) {
            throw new ModelClientException("DuoJie 响应缺少 choices", ModelClientErrorType.INVALID_RESPONSE, null);
        }
        JsonArray choices = respJson.getAsJsonArray("choices");
        if (choices == null || choices.isEmpty()) {
            throw new ModelClientException("DuoJie 响应 choices 为空", ModelClientErrorType.INVALID_RESPONSE, null);
        }
        JsonObject choice0 = choices.get(0).getAsJsonObject();
        if (choice0 == null || !choice0.has("message")) {
            throw new ModelClientException("DuoJie 响应缺少 message", ModelClientErrorType.INVALID_RESPONSE, null);
        }
        JsonObject message = choice0.getAsJsonObject("message");
        if (message == null || !message.has("content") || message.get("content").isJsonNull()) {
            throw new ModelClientException("DuoJie 响应缺少 content", ModelClientErrorType.INVALID_RESPONSE, null);
        }
